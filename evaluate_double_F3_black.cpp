...
int evaluate_double_F3_black(const int(*map)[15], int x, int y) {
	...
	//if (map[x][y] != 1) return 0;// 如果有子直接退出 ，以后还会再优化吧
...
if (is_legal(x, y, 4, DOWNRIGHT)) {
	if (downright(1) == BLANK && downright(2) == CHA_A && downright(3) == CHA_A && downright(4) == CHA_A)
		...
		// paa*a
		if (is_legal(x, y, 4, UP)) {
			if (up(3) == BLANK && up(2) == CHA_A && up(1) == CHA_A && up(4) == CHA_A)
				d_summary[UP][SA4]++;
		}
		if (is_legal(x, y, 4, DOWN)) {
			if (down(3) == BLANK && down(2) == CHA_A && down(1) == CHA_A && down(4) == CHA_A)
				d_summary[DOWN][SA4]++;
		}
		if (is_legal(x, y, 4, LEFT)) {
			if (left(3) == BLANK && left(2) == CHA_A && left(1) == CHA_A && left(4) == CHA_A)
				d_summary[LEFT][SA4]++;
		}
		if (is_legal(x, y, 4, RIGHT)) {
			if (right(3) == BLANK && right(2) == CHA_A && right(1) == CHA_A && right(4) == CHA_A)
				d_summary[RIGHT][SA4]++;
		}
		if (is_legal(x, y, 4, UPLEFT)) {
			if (upleft(3) == BLANK && upleft(2) == CHA_A && upleft(1) == CHA_A && upleft(4) == CHA_A)
				d_summary[UPLEFT][SA4]++;
		}
		if (is_legal(x, y, 4, UPRIGHT)) {
			if (upright(3) == BLANK && upright(2) == CHA_A && upright(1) == CHA_A && upright(4) == CHA_A)
				d_summary[UPRIGHT][SA4]++;
		}
		if (is_legal(x, y, 4, DOWNLEFT)) {
			if (downleft(3) == BLANK && downleft(2) == CHA_A && downleft(1) == CHA_A && downleft(4) == CHA_A)
				d_summary[DOWNLEFT][SA4]++;
		}
		if (is_legal(x, y, 4, DOWNRIGHT)) {
			if (downright(3) == BLANK && downright(2) == CHA_A && downright(1) == CHA_A && downright(4) == CHA_A)
				d_summary[DOWNRIGHT][SA4]++;
		}
		//----------------------------------------------------*&^%*&^%*&^%*&^%*&------------------------------
		...
		// pa*aa 冲4
		if (is_legal(x, y, 4, UP)) {
			if (up(2) == BLANK && up(3) == CHA_A && up(1) == CHA_A && up(4) == CHA_A)
				d_summary[UP][SA4]++;
		}
		if (is_legal(x, y, 4, DOWN)) {
			if (down(2) == BLANK && down(3) == CHA_A && down(1) == CHA_A && down(4) == CHA_A)
				d_summary[DOWN][SA4]++;
		}
		if (is_legal(x, y, 4, LEFT)) {
			if (left(2) == BLANK && left(3) == CHA_A && left(1) == CHA_A && left(4) == CHA_A)
				d_summary[LEFT][SA4]++;
		}
		if (is_legal(x, y, 4, RIGHT)) {
			if (right(2) == BLANK && right(3) == CHA_A && right(1) == CHA_A && right(4) == CHA_A)
				d_summary[RIGHT][SA4]++;
		}
		if (is_legal(x, y, 4, UPLEFT)) {
			if (upleft(2) == BLANK && upleft(3) == CHA_A && upleft(1) == CHA_A && upleft(4) == CHA_A)
				d_summary[UPLEFT][SA4]++;
		}
		if (is_legal(x, y, 4, UPRIGHT)) {
			if (upright(2) == BLANK && upright(3) == CHA_A && upright(1) == CHA_A && upright(4) == CHA_A)
				d_summary[UPRIGHT][SA4]++;
		}
		if (is_legal(x, y, 4, DOWNLEFT)) {
			if (downleft(2) == BLANK && downleft(3) == CHA_A && downleft(1) == CHA_A && downleft(4) == CHA_A)
				d_summary[DOWNLEFT][SA4]++;
		}
		if (is_legal(x, y, 4, DOWNRIGHT)) {
			if (downright(2) == BLANK && downright(3) == CHA_A && downright(1) == CHA_A && downright(4) == CHA_A)
				d_summary[DOWNRIGHT][SA4]++;
		}
		//-------------------------------------------)(*(^^&%&%*%$*$))-----------------------------------------------
		// aapa
		if (is_legal(x, y, 2, UP)&& is_legal(x, y, 1, DOWN)) {
			if (up(1) == CHA_A && up(2) == CHA_A &&up(-1) == CHA_A)
				d_summary[UP][SA4]++;
		}
		if (is_legal(x, y, 2, DOWN) && is_legal(x, y, 1, UP)) {
			if (down(1) == CHA_A && down(2) == CHA_A && down(-1) == CHA_A)
				d_summary[DOWN][SA4]++;
		}
		if (is_legal(x, y, 2, LEFT) && is_legal(x, y, 1, RIGHT)) {
			if (left(1) == CHA_A && left(2) == CHA_A && left(-1) == CHA_A)
				d_summary[LEFT][SA4]++;
		}
		if (is_legal(x, y, 2, RIGHT) && is_legal(x, y, 1, LEFT)) {
			if (right(1) == CHA_A && right(2) == CHA_A && right(-1) == CHA_A)
				d_summary[RIGHT][SA4]++;
		}
		if (is_legal(x, y, 2, UPRIGHT) && is_legal(x, y, 1, DOWNLEFT)) {
			if (upright(1) == CHA_A && upright(2) == CHA_A && upright(-1) == CHA_A)
				d_summary[UPRIGHT][SA4]++;
		}
		if (is_legal(x, y, 2, DOWNLEFT) && is_legal(x, y, 1, UPRIGHT)) {
			if (downleft(1) == CHA_A && downleft(2) == CHA_A && downleft(-1) == CHA_A)
				d_summary[DOWNLEFT][SA4]++;
		}
		if (is_legal(x, y, 2, UPLEFT) && is_legal(x, y, 1, DOWNRIGHT)) {
			if (upleft(1) == CHA_A && upleft(2) == CHA_A && upleft(-1) == CHA_A)
				d_summary[UPLEFT][SA4]++;
		}
		if (is_legal(x, y, 2, DOWNRIGHT) && is_legal(x, y, 1, UPLEFT)) {
			if (downright(1) == CHA_A && downright(2) == CHA_A && downright(-1) == CHA_A)
				d_summary[DOWNRIGHT][SA4]++;
		}
		//-------------------------------------------------------------------(*^*&(^%&^%(*&(*&^(*&^(*&=============================
		{//`a*paa`  冲4
			if (is_legal(x, y, 2, UP) && is_legal(x, y, 2, DOWN)) {
				if (up(1) == CHA_A && up(2) == CHA_A && up(-1) == BLANK && up(-2) == CHA_A)
					d_summary[UP][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, UP) && is_legal(x, y, 1, DOWN)) {
				if (up(1) == BLANK && up(2) == CHA_A && up(3) == CHA_A && up(-1) == CHA_A)
					d_summary[UP][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, DOWN) && is_legal(x, y, 2, UP)) {
				if (down(1) == CHA_A && down(2) == CHA_A && down(-1) == BLANK && down(-2) == CHA_A)
					d_summary[DOWN][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, DOWN) && is_legal(x, y, 1, UP)) {
				if (down(1) == BLANK && down(2) == CHA_A && down(3) == CHA_A && down(-1) == CHA_A)
					d_summary[DOWN][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, LEFT) && is_legal(x, y, 2, RIGHT)) {
				if (left(1) == CHA_A && left(2) == CHA_A && left(-1) == BLANK && left(-2) == CHA_A)
					d_summary[LEFT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, LEFT) && is_legal(x, y, 1, RIGHT)) {
				if (left(1) == BLANK && left(2) == CHA_A && left(3) == CHA_A && left(-1) == CHA_A)
					d_summary[LEFT][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, RIGHT) && is_legal(x, y, 2, LEFT)) {
				if (right(1) == CHA_A && right(2) == CHA_A && right(-1) == BLANK && right(-2) == CHA_A)
					d_summary[RIGHT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, RIGHT) && is_legal(x, y, 1, LEFT)) {
				if (right(1) == BLANK && right(2) == CHA_A && right(3) == CHA_A && right(-1) == CHA_A)
					d_summary[RIGHT][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, UPRIGHT) && is_legal(x, y, 2, DOWNLEFT)) {
				if (upright(1) == CHA_A && upright(2) == CHA_A && upright(-1) == BLANK && upright(-2) == CHA_A)
					d_summary[UPRIGHT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, UPRIGHT) && is_legal(x, y, 1, DOWNLEFT)) {
				if (upright(1) == BLANK && upright(2) == CHA_A && upright(3) == CHA_A && upright(-1) == CHA_A)
					d_summary[UPRIGHT][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, UPLEFT) && is_legal(x, y, 2, DOWNRIGHT)) {
				if (upleft(1) == CHA_A && upleft(2) == CHA_A && upleft(-1) == BLANK && upleft(-2) == CHA_A)
					d_summary[UPLEFT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, UPLEFT) && is_legal(x, y, 1, DOWNRIGHT)) {
				if (upleft(1) == BLANK && upleft(2) == CHA_A && upleft(3) == CHA_A && upleft(-1) == CHA_A)
					d_summary[UPLEFT][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, DOWNLEFT) && is_legal(x, y, 2, UPRIGHT)) {
				if (downleft(1) == CHA_A && downleft(2) == CHA_A && downleft(-1) == BLANK && downleft(-2) == CHA_A)
					d_summary[DOWNLEFT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, DOWNLEFT) && is_legal(x, y, 1, UPRIGHT)) {
				if (downleft(1) == BLANK && downleft(2) == CHA_A && downleft(3) == CHA_A && downleft(-1) == CHA_A)
					d_summary[DOWNLEFT][SA4]++;
			}
			...
			//`a*paa`		冲4
			if (is_legal(x, y, 2, DOWNRIGHT) && is_legal(x, y, 2, UPLEFT)) {
				if (downright(1) == CHA_A && downright(2) == CHA_A && downright(-1) == BLANK && downright(-2) == CHA_A)
					d_summary[DOWNRIGHT][SA4]++;
			}

			//`ap*aa`		冲4
			if (is_legal(x, y, 3, DOWNRIGHT) && is_legal(x, y, 1, UPLEFT)) {
				if (downright(1) == BLANK && downright(2) == CHA_A && downright(3) == CHA_A && downright(-1) == CHA_A)
					d_summary[DOWNRIGHT][SA4]++;
			}
